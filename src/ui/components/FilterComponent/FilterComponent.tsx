...
export default Vue.extend({
	...
	data() {
		...
		const beforeYesterday = beforeYesterdayYear+'-'+
		(beforeYesterdayMonth>9?beforeYesterdayMonth:'0'+beforeYesterdayMonth)+'-'+
	...
	methods: {
		...
		setFilterClassifier(c: Classifier): void {
			this.filter.classifier = c.classifierName
			this.filter.class = ''
		},
		...
		setDateTextField(): string | void {
			...
			if (julian0<julian1) {
				return date[0]+' ~ '+date[1] + ' ('+julian0+' ~ '+julian1+')'
			}
		...
		async fetchCandidates() {
			...
			await this.$store.dispatch('selected/setCandidate', this.$store.getters['candidate/get'][0]);
		...
		const renderClasses = () => {
			...
			return (
				<v-select
					...
					label='Class'
					items={
						this.$store.getters['classifier/get']
						.filter(
							(c: Classifier) => c.classifierName === this.filter.classifier
						)[0].classes
					}
					item-text='classes'
